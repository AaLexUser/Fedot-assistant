...
class PredictionTask:
    ...
    def __init__(
        ...
        print(f"Task name: {self.metadata['name']}")
        print(f"Task description: {self.metadata['description']}")
    ...
    def from_path(
        ...
        if not task_root_dir.exists() or not task_root_dir.is_dir():
            raise ValueError(
                f"Task directory {task_root_dir} does not exist or is not a directory"
            )
        ...
        if not task_data_filenames:
            raise ValueError(f"No files found in task directory: {task_root_dir}")

        return cls(
            filepaths=[task_root_dir / fn for fn in task_data_filenames],
            metadata=dict(
                name=name if name is not None else task_root_dir.name,
                description=description,
            ),
        )
    ...
    def _set_task_files(
        ...
        for k, v in dataset_name_mapping.items():
            if v is None:
                ...
                filepath = next(
                    iter(
                        [path for path in self.filepaths if path.name == Path(v).name]
                    ),
