...
from fedotllm.llm import AssistantChatOpenAI, clip_text
from fedotllm.prompting import (
    ...
    def transform(self, task: PredictionTask) -> PredictionTask:
        ...
        if descriptions_read:
            if task.metadata["description"]:
                task.metadata["description"] = clip_text(
                    task.metadata["description"] + "\n\n" + descriptions_read,
                    max_tokens=self.llm.max_input_tokens,
                )
            else:
                task.metadata["description"] = clip_text(
                    descriptions_read, max_tokens=self.llm.max_input_tokens
                )
