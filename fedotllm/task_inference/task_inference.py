...
class TaskInference:
    ...
    def log_value(self, key: str, value: Any, max_width: int = 1600) -> None:
        ...
        if not value:
            logger.info(f"WARMING: Failed to identify the {key} of the task, it is set to None.")
            return
    ...
    def _chat_and_parse_prompt_output(self) -> Dict[str, str]:
        try:
            assert self.prompt_genetator is not None, "prompt_generator is not initialized"
            ...
            parsed_output = self.prompt_genetator.parser(output,
...
class DescriptionFileNameInference(TaskInference):
    ...
    def _read_descriptions(self, parser_output: Dict[str, Union[str, Iterable[str]]]) -> str:
        description_parts = []
...
class ProblemTypeInference(TaskInference):
    def initialize_task(self, task):
        ...
        self.prompt_genetator = ProblemTypePromptGenerator(data_description=task.metadata["description"])
    ...
    def post_process(self, task, value):
        ...
        if value in CLASSIFICATION_PROBLEM_TYPES:
            problem_type_infered_by_autogloun = infer_problem_type(task.train_data[task.label_column], silent=True)
